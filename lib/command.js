...
function spawn(command, args, options, callback) {
  ...
  childProcObj.stderr.on('data', function (data) {
    if (! stderr) process.stdout.write(options.logPrefix);
    process.stdout.write(utils.prefixLines(options.logPrefix, data.toString()));
    stderr += data.toString();
  }.bind(this));

  // Handle stdout.
  childProcObj.stdout.on('data', function (data) {
    if (! stdout) process.stdout.write(options.logPrefix);
    process.stdout.write(utils.prefixLines(options.logPrefix, data.toString()));
